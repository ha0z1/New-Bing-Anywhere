...
本站自身無後端。另有一個**可選**的賬號服務跑在 `api.tmux.online`（倉庫在 `../AA-Server`），只服務於 `/account` 和 `/device` 兩頁；首頁和安裝流程完全不依賴它。
...
src/components/Account.astro     /account 頁：預渲染的外殼，包著島嶼
src/components/Device.astro      /device 頁：同樣結構
src/islands/AccountPanel.tsx     React：登錄態、登入登出、API key
...
src/pages/zh-Hant/          /zh-Hant   → 同樣三頁的繁中版
...
- **語言寫進根域 cookie `L`**（`L=zh-Hant`，`domain=.tmux.online`、`path=/`、一年、`SameSite=Lax`），供 tmux.online 下的其他服務直接讀。在客戶端寫（`src/components/LangCookie.astro`），不從 Worker 發 `Set-Cookie`——帶 Set-Cookie 的響應在邊緣不可緩存。**本站自己不讀這個 cookie**：渲染哪種語言只由 URL 決定，這樣過期的 cookie 永遠不會跟路徑打架、也不會讓緩存頁面出錯。
...
- **只有 `/account` 和 `/device` 水合**。全部頁面都是構建時預渲染的 HTML，React 只接管這兩個有狀態的界面。營銷頁必須一個框架字節都不發——改完用 `grep -c astro-island dist/index.html`（應為 0）核對。
...
- **頭部賬號入口是固定 30px 圓**：未登錄一個用戶圖標、登錄後頭像填滿，兩態同寬所以 session 回來不會讓導航欄回流（別再往裏加暱稱等變寬的東西）。頭像 URL 緩存在 `localStorage['aa_avatar']`，刷新時先樂觀畫出來、session 回來再對賬（登出則還原成圖標並清緩存）。
...
- **文案的紅線**：首頁多處寫過「不需要賬號、不用登錄」。這些話現在的正確形態是「登錄是可選的」，不是刪掉。核心功能確實不需要賬號，改文案時別把這個事實一起改沒了。`src/lib/llms.ts` 裏有一段**硬編碼的英文散文**（唯一一處文案不在 i18n 表裏的地方），改賬號相關措辭時它要同步改。
- **`/account` 和 `/device` 必須保持 noindex**：`<Base noindex>`（同時會跳過 JSON-LD 和 hreflang）、`public/_headers`、`astro.config.mjs` 的 sitemap `filter`，三處都要有。`/device` 的 URL 上帶一次性設備碼，尤其不能進索引。
- **設備碼授權的順序不能動**：`GET /api/auth/device` 是唯一把設備碼綁定到用戶的動作，且只有在該請求帶著 session cookie 時才算數。跳過它，approve 就會以一個看不懂的 400 失敗。better-auth 文檔裏那個 `/login?redirect=` 路由是 Next.js 的示例，不是庫的要求——這個端點返回 JSON，從不重定向。
