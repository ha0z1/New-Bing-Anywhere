...
The site itself has no backend. There is a separate, entirely optional account service at
`api.tmux.online` (its repository is `../AA-Server`) which two pages here talk to: `/account`
and `/device`. Nothing on the landing page or in the install flow depends on it.
...
src/components/Account.astro     /account — prerendered shell around the island
src/components/Device.astro      /device  — same shape
src/islands/AccountPanel.tsx     React: session, sign in/out, API keys
...
src/pages/account.astro      /account   → Account lang="en"   (noindex)
src/pages/device.astro       /device    → Device lang="en"    (noindex)
src/pages/zh-Hant/           /zh-Hant   → the same three pages, 繁體中文
...
Every page is prerendered to HTML at build time. React hydrates exactly two surfaces —
`/account` and `/device` — because those depend on a session and on `?user_code=`, and
nothing else here has state worth keeping. The islands are passed their copy as props, so a
page ships one locale, not all of them.
...
- **The header must not import `src/lib/api.ts`.** It renders on the landing page, and that
  ...
  reuses so the account page asks for the session once rather than twice.
...
Worker: a response carrying `Set-Cookie` is not edge-cacheable, and this site reads the
value back nowhere. Off the real domain — localhost, `astro preview`, a Workers preview URL
— the `Domain` attribute is omitted and the cookie is host-only, because a `Domain` naming
a host you are not on is rejected outright.

Nothing on this site _reads_ the cookie: the URL is the source of truth for which language
is rendered, so a stale cookie can never fight the path or make a cached page wrong.

## The account pages

`/account` and `/device` are the only pages that talk to a server, and the only ones that
hydrate. They call `api.tmux.online` with plain `fetch` from `src/lib/api.ts`. There is
...
Both pages are `noindex` in three places — the `noindex` prop on `Base` (which also drops the
JSON-LD and the hreflang alternates), an `X-Robots-Tag` in `public/_headers`, and the sitemap
`filter` in `astro.config.mjs`. `/device` in particular carries a one-time device code in its
query string and has no business in a search index.
...
`public/install.sh` is currently a **placeholder** — it installs nothing and exits 1 with a
pointer to the repository. Replace the file wholesale when the real installer is ready;
`_headers` already serves it as `text/plain` with a 5-minute cache so an update goes live
quickly.
