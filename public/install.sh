...
as_root() {
  if [ "$(id -u)" -eq 0 ]; then
    ...
    die "need root to install packages, and sudo is not available — re-run this as root"
  fi
}
...
ensure_tmux() {
  ...
  have tmux || die "tmux install finished but tmux is still not on PATH"
...
ensure_node() {
  ...
  have node || die "the version manager finished but node is still not on PATH — open a new shell and re-run, or install Node from https://nodejs.org"
  node_ok || die "the version manager installed $(node -v), which is below the required ${NODE_MIN_MAJOR}.${NODE_MIN_MINOR} — an older node may be earlier in PATH. Install Node from https://nodejs.org and re-run."
...
install_cli() {
  ...
  npm -v >/dev/null 2>&1 || die "npm is on PATH at $(command -v npm) but does not run — install your distro's npm package for Node ${NODE_MIN_MAJOR}, then re-run"
  say "installing ${PKG}…"
  ...
  if [ -n "$_prefix" ] && [ -w "$_prefix" ]; then
    ...
    as_root npm install -g "$PKG@latest"
  fi
  if have ai-anywhere; then
    CLI_ON_PATH=1
    ok "installed: $(command -v ai-anywhere)"
  else
...
printf '\n'
if [ "$CLI_ON_PATH" -eq 1 ]; then
  ok "${B}All set.${Z} Start the server with:  ${B}ai-anywhere up${Z}"
  say "Then open the URL it prints to authorise this machine in your browser."
else
  # Don't hand out a command that this shell cannot run — point at the one it can.
  ok "${B}Installed.${Z} Fix PATH as above, then start the server with:  ${B}ai-anywhere up${Z}"
fi
