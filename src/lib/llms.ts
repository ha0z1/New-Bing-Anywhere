...
user's own machines: ${c.brand.name} serves this static site and the install script, plus an
optional account service at ${API_URL.replace('https://', '')}. There is no relay and no
telemetry, and no account is needed to install or use it — signing in exists only to authorise
a CLI on a machine without a browser and to issue API keys for scripts.
...
- Server package: \`ai-anywhere-server\` (npm), default port 16666, bound to 127.0.0.1
- Interface: the server hosts the web UI at http://127.0.0.1:16666; a Chrome extension adds
  the same workspace as a side panel plus a page element picker
- Model: one tmux window is one task in the left rail, the panes inside it are the tabs
  across the top; ssh hosts appear as their own groups in the same rail
- AI CLIs detected and run as tasks: claude, codex, gemini, aider, copilot
...
- [npm](${NPM_URL}): the \`ai-anywhere-server\` package
