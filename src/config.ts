...
export const NPM_URL = 'https://www.npmjs.com/package/ai-anywhere-server'
export const INSTALL_URL = '/install.sh'

/**
 * The install one-liners. These are shell, not prose — identical in every locale — so they
 * live here rather than in the i18n copy tables. `wget` is first, which makes it the default
...
/**
 * The optional account service. Same registrable domain as the site, which is what lets a
...
/**
 * Language preference, written to a cookie on the registrable domain so that anything else
 * under tmux.online — an API on a sibling subdomain, say — reads the same value without
 * having to parse the URL or guess from Accept-Language. Value is a locale code from
 * `src/i18n`, e.g. `L=zh-Hant`.
 *
